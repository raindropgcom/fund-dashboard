...
for r in wb2['Sheet1'].iter_rows(min_row=2, values_only=True):
    ...
    if str(r[0]).strip() in ("006503", "6503"):
...
for r in rows:
    ...
    if st and "撤单" in str(st): continue
    if "转入" in bt:
...
# 3) 严格按日 XIRR
cfs = [(d, cash) for d, _, cash, _ in ledger] + [(VAL_DATE, cur_value)]
d0 = cfs[0][0]
def xnpv(rate):
...
# 平均资金占用(元) = Σ(净流出×天数)/总天数
total_days = (VAL_DATE - d0).days
cap_days = sum(-cash*(VAL_DATE-d).days for d,_,cash,_ in ledger)  # 元*天
avg_capital = cap_days / total_days
simple = cur_value/net - 1

print("="*60)
print("006503 资金占用内在回报率(严格按日 XIRR)")
print("="*60)
print(f"逐笔现金流笔数: {len(ledger)}  区间: {d0} ~ {VAL_DATE} ({total_days} 天)")
print(f"累计买入: {total_buy:.2f}  累计卖出: {total_sell:.2f}  净投入: {net:.2f}")
print(f"当前现值: {cur_value:.2f}  实际利润: {profit:.2f}")
print(f"期末份额: {shares:.2f}  隐含净值: {cur_value/shares:.4f}")
...
print(f"{str(VAL_DATE):<12}{'现值':<6}{cur_value:>12.2f}{0:>8}{0:>14.1f}")

json.dump({
    "code":"006503","xirr":xirr,"roots":roots,
    "total_buy":total_buy,"total_sell":total_sell,"net":net,
    "cur_value":cur_value,"profit":profit,"shares":shares,
    "first_date":str(d0),"val_date":str(VAL_DATE),"days":total_days,
    "avg_capital":avg_capital,"simple":simple,
    "ledger":[[str(d),tp,cash,days,cd] for d,tp,cash,days,cd in occ],
}, open(r"E:\AKshare\_006503_daily.json","w",encoding="utf-8"), ensure_ascii=False, indent=2)
print("\n已保存 _006503_daily.json")
